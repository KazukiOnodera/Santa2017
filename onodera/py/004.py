...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec 17 13:51:12 2017

@author: konodera

nohup python -u 004.py 4 > log004_4.txt &
nohup python -u 004.py 5 > log004_5.txt &
nohup python -u 004.py 6 > log004_6.txt &
nohup python -u 004.py 7 > log004_7.txt &
...
while True:
    ...
    cids = np.random.choice(np.arange(4000, 1000000), replace=False, size=30000)
    callback = pool.map(nottwin_vs_nottwin1, cids)
    pool.close()
    ...
    print("not twins1 shuffle times:{}".format(len(ng_list)/2))
...
#        gifts[k].cids = k_list
    ...
    # twins
    change_twins = get_change_twins()
    pool = Pool(total_proc)
    np.random.shuffle(change_twins)
    callback = pool.map(twin_vs_nottwin, change_twins)
    pool.close()
    callback = sum(callback, [])
    callback = sorted(callback, key=itemgetter(2), reverse=True)
    
    ng_list = []; changed_nottwin = []
    changed_twin_pairs = []
    for (cid1, cid2, d) in callback:
        if cid1 in ng_list or cid2 in ng_list:
            continue
        ng_list.append(cid1); ng_list.append(cid2); changed_nottwin.append(cid2)
        changed_twin_pairs.append((cid1, cid2, d))
        gid1 = children[cid1].gid
        gid2 = children[cid2].gid
        children.replace(cid1, cid2)
        gifts.replace(gid1, gid2, cid1)
...
#    print(hp)
    ...
    # not twins2
    pool = Pool(total_proc)
    np.random.shuffle(changed_nottwin)
#    callback = pool.map(nottwin_vs_nottwin2, np.arange(4000, 1000000))
    callback = pool.map(nottwin_vs_nottwin2, changed_nottwin)
    pool.close()
    callback = sum(callback, [])
    callback = sorted(callback, key=itemgetter(2), reverse=True)
    ...
    for (cid1, cid2, d) in callback:
        if cid1 in ng_list or cid2 in ng_list:
            continue
        changed_pairs.append((cid1, cid2, d))
        ng_list.append(cid1); ng_list.append(cid2)
        gid1 = children[cid1].gid
        gid2 = children[cid2].gid
        children.replace(cid1, cid2)
        gifts.replace(gid1, gid2, cid1)
        gifts.replace(gid2, gid1, cid2)
    cnt_changed = len(ng_list)/2
    changed_nottwin += ng_list
    print("not twins2 shuffle times:{}".format(cnt_changed))
    ...
    while cnt_changed > 0:
        ...
        cnt_changed = len(ng_list)/2
        changed_nottwin += ng_list
        print("not twins3 shuffle times:{}".format(cnt_changed))
