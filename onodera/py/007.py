...
child = pd.read_csv('../input/child_wishlist_v2.csv.zip', header=None)
child.columns = ['cid'] + list(range(1, child.shape[1]))

gift = pd.read_csv('../input/gift_goodkids_v2.csv.zip', header=None)
...
triplets = list(zip(*[iter(range(5001))]*3))
twins = list(zip(*[iter(range(5001,45001))]*2))
def get_brother(x):
    if x <= 5000:
        for ids in triplets:
            if x in ids:
...
class Child:
    ...
    def __init__(self, cid, gid):
        ...
        self.id   = values[0]
        self.brother_ids = get_brother(self.id)
        self.pref = values[1:]
        self.happiness = -1/20
        self.gid = gid
        ...
        prefer = self.pref
        if self.brother_ids==-1:
            for i in range(prefer.shape[0]):
                self.prefer_dict[prefer[i]] = 400*(prefer.shape[0] - i) - 2
                
        elif self.id<=5000:
            prefer2 = get_child(self.brother_ids[0])[1:]
            prefer3 = get_child(self.brother_ids[1])[1:]
            for p in list(set(list(prefer) + list(prefer2)+ list(prefer3))):
                score = 0
                if p in list(prefer):
